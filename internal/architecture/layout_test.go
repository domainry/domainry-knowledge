...
func TestLayeredLayoutAndPrivateImplementations(t *testing.T) {
	root := filepath.Join("..", "..")
	for _, rel := range []string{"internal/application/knowledge", "internal/assembly/module", "internal/infrastructure/persistence/database/knowledge", "internal/infrastructure/persistence/database/record", "internal/infrastructure/provider", "internal/infrastructure/attachmentstorage", "module"} {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, err error) error {
		...
		for _, im := range f.Imports {
			...
			if domain || application || contract {
				...
				if strings.HasPrefix(imported, "github.com/domainry/domainry-knowledge/") {
					local := strings.TrimPrefix(imported, "github.com/domainry/domainry-knowledge/")
			...
			if strings.Contains(imported, "/internal/") && !strings.HasPrefix(imported, "github.com/domainry/domainry-knowledge/") {
