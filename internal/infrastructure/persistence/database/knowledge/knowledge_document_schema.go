...
const CompatKnowledgeDocumentSourceTable = "_agent_knowledge_document_sources"
...
func CompatKnowledgeDocumentMigration(d modulehost.Dialect) (modulehost.SchemaMigration, error) {
	...
	for _, builder := range []*ormschema.TableBuilder{
		ormschema.NewTable(d, CompatKnowledgeDocumentSourceTable).IfNotExists().Columns(required("scope_key", ormschema.TextKey(64)), required("library_id", ormschema.TextKey(96)), required("source_key", ormschema.TextKey(64))).PrimaryKey("scope_key", "library_id").Unique("source_key"),
