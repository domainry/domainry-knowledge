...
func (r *Registrar) ApplyOwnedMigrations(ctx context.Context, owner string, values []modulehost.SchemaMigration) error {
...
func (r *Registrar) apply(ctx context.Context, owner string, value modulehost.SchemaMigration, checksum string) error {
	if strings.TrimSpace(owner) == "" || value.Version == 0 || strings.TrimSpace(value.Name) == "" {
		return fmt.Errorf("invalid Knowledge host migration")
	}
